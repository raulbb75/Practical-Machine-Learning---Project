...
Load the files and install packages, libraries, and setting the seed for reproduceability:
...
# Overall seed for reproduceability
set.seed(1234)
```
...
setwd("C:\\Users\\banaresr\\Documents\\R")

# Loading the training data set into my R session replacing all missing with "NA"
trainingset <- read.csv("C:\\Users\\banaresr\\Documents\\R\\pml-training.csv", na.strings=c("NA","#DIV/0!", ""))

# Loading the testing data set 
testingset <- read.csv('C:\\Users\\banaresr\\Documents\\R\\pml-testing.csv', na.strings=c("NA","#DIV/0!", ""))
...
```{r Pred1_Decision_Tree, message = FALSE, warning = FALSE, fig.width=18, fig.height=10}
...
```{r Pred2_Random_Forest, message = FALSE, warning = FALSE, fig.width=18, fig.height=10}
...
Random Forest algorithm performed better prediction than Decision Trees.

Accuracy for Random Forest model was 0.995 (95% CI: (0.993, 0.997)) compared to 0.739 (95% CI: (0.727, 0.752)) for Decision Tree model. 

The accuracy of Random Forest was 0.995. The expected out-of-sample error is estimated at 0.005, or 0.5%. The expected out-of-sample error is calculated as 1 - accuracy for predictions made against the cross-validation set. Our Test data set comprises 20 cases. With an accuracy above 99% on our cross-validation data, we can expect that very few, or none, of the test samples will be missclassified.

```{r Outcome, message = FALSE, warning = FALSE, fig.width=18, fig.height=10}
